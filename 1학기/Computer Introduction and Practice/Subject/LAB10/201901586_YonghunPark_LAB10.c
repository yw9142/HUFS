...
void ADD(struct BOOK_INFO *BOOK, struct PUBLISHER_INFO *PUBLISHER, struct AUTHOR_INFO *AUTHOR); // prototype declaration

void
DELETE(struct BOOK_INFO *BOOK, struct PUBLISHER_INFO *PUBLISHER, struct AUTHOR_INFO *AUTHOR); // prototype declaration

int
SEARCH(struct BOOK_INFO *BOOK, struct PUBLISHER_INFO *PUBLISHER, struct AUTHOR_INFO *AUTHOR); // prototype declaration

void
UPDATE(struct BOOK_INFO *BOOK, struct PUBLISHER_INFO *PUBLISHER, struct AUTHOR_INFO *AUTHOR); // prototype declaration

void
DISPLAY(struct BOOK_INFO *BOOK, struct PUBLISHER_INFO *PUBLISHER, struct AUTHOR_INFO *AUTHOR); // prototype declaration
...
void sort(BI *BOOK, PI *PUBLISHER, AI *AUTHOR) {
    BI TEMP;
    PI TEMP1;
    AI TEMP2;
...
void ADD(BI *BOOK, PI *PUBLISHER, AI *AUTHOR) {
    int ID, PID, AID;
    ...
    for (int i = 0; i < count; i++) { // Confirm PUBLISHER ID
        if (PUBLISHER[i].Publisher_ID == PID) { // already exists
            bool = 1; // TRUE
    ...
    if (bool == 1) { // if TRUE
        ...
        scanf("%[^\n]s", PUBLISHER[count].Publisher_Name); // If it does not exist, enter new information
        fflush(stdin);

        printf("Enter the Publisher Phone Number : ");
        scanf("%[^\n]s", PUBLISHER[count].Publisher_Number);
        fflush(stdin);
    }
    ...
    int AUTHOR_INDEX = 0;
    for (int i = 0; i < count; i++) { // Confirm PUBLISHER ID
        if (AUTHOR[i].Author_ID == AID) { // already exists
            bool = 1; // TRUE
    ...
    if (bool == 1) {
        ...
        scanf("%[^\n]s", AUTHOR[count].Author_Name);
        fflush(stdin);

        printf("Enter the Author e-amil address : ");
        scanf("%[^\n]s", AUTHOR[count].Author_eaddress);
        fflush(stdin);
    }

    printf("Enter the Book Name : ");
    scanf("%[^\n]s", BOOK[count].Book_Name);
    fflush(stdin);

    printf("Enter the date of publication of a book : ");
    scanf("%[^\n]s", BOOK[count].Book_date_of_issue);
    fflush(stdin);

    count++;
}

void DELETE(BI *BOOK, PI *PUBLISHER, AI *AUTHOR) {
    int index;
...
int SEARCH(BI *BOOK, PI *PUBLISHER, AI *AUTHOR) {
    int ID;
...
void UPDATE(BI *BOOK, PI *PUBLISHER, AI *AUTHOR) {
    ...
    if (bool == 1) {
        ...
        printf("Enter the Publisher Name : "); // Set value in current index based on global variable
        scanf("%[^\n]s", PUBLISHER[index].Publisher_Name);
        fflush(stdin);

        printf("Enter the Publisher Phone Number : ");
        scanf("%[^\n]s", PUBLISHER[index].Publisher_Number);
        fflush(stdin);
    }
    ...
    printf("Enter the Book Name : ");
    scanf("%[^\n]s", BOOK[index].Book_Name);
    fflush(stdin);

    printf("Enter the date of publication of a book : ");
    scanf("%[^\n]s", BOOK[index].Book_date_of_issue);
    fflush(stdin);
}

void DISPLAY(BI *BOOK, PI *PUBLISHER, AI *AUTHOR) {
    int num;
