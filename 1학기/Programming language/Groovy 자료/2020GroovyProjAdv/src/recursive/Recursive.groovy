...
def merge(List L1, List L2, Closure comp) {
	if (L1.size()==0)
		L2
	else if (L2.size()==0)
		L1
		...
		def y = L2.head()
		if (comp(x, y)<0)
			[x]+merge(L1.tail(), L2, comp)
		else
			[y]+merge(L1, L2.tail(), comp)
	}
}

def mergeSort(List list, Closure comp) {
	def n = list.size()
	if (n<=1)
		list
	else {
		...
		def L1 = list[0 .. n2 - 1]
		def L2 = list[n2 .. n - 1]
		merge(mergeSort(L1, comp), mergeSort(L2, comp), comp)
	}
}

comp = { x,y -> x<=>y }
comp1 = { x,y -> x.toString()<=>y.toString() }
comp2 = { x,y -> (x<=>y) * (-1) }
println "mergeSort([2,4,6,10,3,5,1], comp1)=${mergeSort([2,4,6,10,3,5,1], comp1)}"
println "mergeSort([2,4,6,10,3,5,1], comp2)=${mergeSort([2,4,6,10,3,5,1], comp2)}"
