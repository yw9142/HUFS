...
void DISPLAY(void) { // 디스플레이 함수.
    ...
    if (num == 1) { // 작가명을 기준으로 확인
        printf("Enter the author name : ");
        scanf("%s", author);
        findauthor((char) *author); // display함수에서 매개변수를 보낼때 애스터리스크로 보낸 이유는 char형이라 보낼때 이상하게 매개변수 전달이 되서 if문에 걸리지 않는 오류가 발생했음.
    } else if (num == 2) { // 출판사명을 기준으로 확인.
        printf("Enter the Publisher name : ");
        scanf("%s", publisher);
        findpublisher((char) *publisher); // display함수에서 매개변수를 보낼때 애스터리스크로 보낸 이유는 char형이라 보낼때 이상하게 매개변수 전달이 되서 if문에 걸리지 않는 오류가 발생했음.
